...
## Input Options
The guidance supports RTP Push, RTMP push, HLS and [AWS Elemental Link](https://aws.amazon.com/medialive/features/link/) input types. For more detialed instructions see the implementation guide on the [guidance home page](https://aws.amazon.com/guidances/live-streaming-on-aws/). 
...
The CloudFormation template (generated by the CDK) includes a lambda backed custom resource to configure MediaLive and create a UUID. To launch the guidance the Lambda source code has to be deployed to an Amazon S3 bucket in the region you intend to deploy the guidance. 
...
When deploying this guidance you may have a DevOps operator IAM user that deploys this CloudFormation template. The json file located at the following directory is the minimum IAM permissions that an AWS IAM user needs to be able to deploy the CloudFormation template successfully. 
